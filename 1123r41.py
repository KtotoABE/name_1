...
def add_absence_date():
    ...
    absence_date_input = input("Enter absence date (YYYY-MM-DD HH:MM:SS) or leave empty for today: ")
    absence_date_input = absence_date_input or datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    if not is_valid_date(absence_date_input): return print("Invalid date format.")
    if datetime.strptime(absence_date_input, "%Y-%m-%d %H:%M:%S") < datetime.now():
...
def add_work_date():
    ...
    work_date_input = input("Enter work date (YYYY-MM-DD HH:MM:SS): ")
